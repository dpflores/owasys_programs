...
void GetFullGPSPosition( void )
{
   ...
   if( ReturnCode != NO_ERROR )
      printf( "Error %d in GPS_GetAllPositionData()...\n", ReturnCode);
		...
		printf("{\"pos_valid\": %hhu, \"old_pos\": %hhu, \"total\": %d, \"nav_status\": %s, \"lat\": %.7lf, \"lon\": %.7lf, \"alt\": %04.03f, \"hAcc\": %04.01f, \"vAcc\": %04.01f, \"Speed\": %04.03f, \"Course\": %04.02f, \"HDOP\": %04.03f, \"VDOP\": %04.01f, \"TDOP\": %04.01f, \"numSvs\": %hhu}\r",
